    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <input class="btn btn-success" onclick="getFormSendGroup()" value="Create Group"
                           data-dismiss="modal">
...
<div
    ...
    <ul>
        ...
        <li>
            <button class="btn btn-danger " style="width: 140px" id="online"> Connecting...</button>
        </li>
    </ul>
    ...
    <ul>
        <li id="option">
            <ul class="list-group">
                <li class="list-group-item list-group-item-success" style="font-size: 14px;font-weight: 400;"><b
                            onclick="Member_Info()"><i class="glyphicon glyphicon-refresh"></i> Refresh </b></li>
                <li class="list-group-item list-group-item-danger" data-toggle="modal" data-target="#myModal"
                    style="font-size: 14px;font-weight: 400;"><i class="glyphicon glyphicon-cloud"></i>Create Group
                </li>
            </ul>
        </li>
        <li>
            <button class="btn btn-info" style="width: 140px" onclick="showOption()"><i
                        class="glyphicon glyphicon-cog"></i> Option
            </button>
...
<script>
    ...
    function sendEventPresence(message, NameChannel, checkChannel, NameChannelGroup=null, ListUser=null) {
        ...
        NameChannel = NameChannel.replace(' ', '-');
        checkChannel = checkChannel.replace(' ', '-');
                               ...
                               $('#list-message-' + Name_ChannelSend_Private).append(data2[1]).scrollTop(9999);
                                ...
                                for (i = 0; i < ListChannel.length; i++) {
                                    if (ListChannel[i] === NameChannel) {
                                        j = 1;
                                        console.log('INFOR:channel existed!');
                                        break;
                                    }
                                }
                                if (j === 0) {
                                    console.log('INOFOR:channel listen');
                                    ...
                                    PrivateChannel.bind("Minh\\PusherChat\\Event\\SentMessage", function (data) {
                                        if ($('#message-content-' + Name_ChannelSend_Private).attr('class') == null) {
                                            console.log('INFOR:create form send');
                                            $.get('FormSend/' + message + '/' + Name_ChannelSend_Private, function (data) {
                                                var data2 = data.split("+++");
                                                $('#chat-message').append(data2[0]);
                                                $('#list-message-' + Name_ChannelSend_Private).append(data2[1]).scrollTop(9999);
                                            ...
                                            if (data.user.name != '{{auth::user()->name}}') {
                                                console.log('INFOR: insert message');
